...
def fitTadin(sizes, peaks, contrasts, peaksSEM, startingParams, paramULs, paramLLs):
    ...
    def spatialSuppressionMechanisticModel(sizesXcontrast, alpha, beta, Ae, Ai, ne, ni, c50e, c50i, w50e, w50i, me, mi, R0, C):
        ...
        for size in sizes:
            for contrast in contrasts:
                ...
                E = (size**me)/(size**me + w50e**me)

                I = 1-np.e**(-((size**2)/(beta**2))/2)
                I = (size**mi)/(size**mi + w50i**mi)
    ...
    ne = startingParams[6]
    ni = startingParams[7]
    w50e = startingParams[8]
    w50i = startingParams[9]
    me = startingParams[10]
    mi = startingParams[11]
    ...
    p0 = [alpha, beta, Ae, Ai, ne, ni, c50e, c50i, w50e, w50i, me, mi, R0, C]
    diff = 0.001
    lowerLims = [alpha_ll, beta_ll, Ae_ll, Ai_ll, ne_ll-diff, ni_ll-diff, c50e_ll-diff, c50i_ll-diff, w50e_ll-diff, w50i_ll-diff, me_ll-diff, mi_ll - diff, 0, 0]
    upperLims = [alpha_ul, beta_ul, Ae_ul, Ai_ul, ne_ul+diff, ni_ul+diff, c50e_ul+diff, c50i_ul+diff, w50e_ul+diff, w50i_ul+diff, me_ul+diff, me_ul+diff, 10, 10]
...
#        plt.plot(Rs[:, 1])
    ...
    popt, pcov = curve_fit(spatialSuppressionMechanisticModel, (sizes_repackaged,contrasts_repackaged), peaks_vector, p0=p0, maxfev=100000, bounds=(lowerLims, upperLims))
 ...
 #   plt.plot(np.log(sizes), peaks['Contrast0.07'])
    ...
    - me:   ' + str(round(me_fit,3)) + '   (' + str(me_ll) + ', ' + str(me_ul) + ')\n\
    - w50e:   ' + str(round(w50e_fit,3)) + '   (' + str(w50e_ll) + ', ' + str(w50e_ul) + ')\n\
...
Inhibitory Surround:\n\
    - mi:   ' + str(round(mi_fit,3)) + '   (' + str(mi_ll) + ', ' + str(mi_ul) + ')\n\
    - w50i:   ' + str(round(w50i_fit,3)) + '   (' + str(w50i_ll) + ', ' + str(w50i_ul) + ')\n\
    ...
    Iws = []
    for w in ws:
        Ew = (w**me_fit)/(w**me_fit + w50e_fit**me_fit)
        Ews.append(Ew)
        Iw = (w ** mi_fit) / (w ** mi_fit + w50i_fit ** mi_fit)

        Iws.append(Iw)

    plt.plot(np.log(ws), Ews, label='E', color='k', linestyle='-')
    plt.plot(np.log(ws), Iws, label='I', color='k', linestyle='--')
    ...
    for c in cs:
        c = c/100
        Ke = ((c ** ne_fit) / (c ** ne_fit + c50e_fit ** ne_fit))
        Kes.append(Ke)

        Ki = ((c ** ni_fit) / (c ** ni_fit + c50i_fit ** ni_fit))
        Kis.append(Ki)




    plt.plot(cs, Kes, label='E', color='k', linestyle='-')
    plt.plot(cs, Kis, label='I', color='k', linestyle='--')

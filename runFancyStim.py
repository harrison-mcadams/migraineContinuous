def makeMotionDiscrimination(trialParams):
    ...
    def makeBackgroundCoordinates(dotSize, backgroundSize):
    ...
    nRandomIterations = 10
    circleIndicesToIterate = []
    nDots = len(circle_xys)
    for ii in range(nRandomIterations):
    ...
    def randomizeColors(initialColorArray, proportionToPresere):

        nDots = len(initialColorArray)
        ...
        newColorArray = initialColorArray * np.rot90(randomizeArray)
    ...
    circle = visual.ElementArrayStim(win=mywin, units='pixels',
                                     ...
                                     elementMask='circle',
                                     xys=circle_xys,
                                     sizes=dotSize_pixels * 1,
                                     colors=colors,
                                     fieldPos=[0, 0]
                                     )

    verticesBase = circle.verticesBase
    verticesPix = circle.verticesPix
    sizes = circle.sizes
    oris = circle.oris
    opacities = circle.opacities
    sfs = circle.sfs
    phases = circle.phases

    backgroundScaleFactor = 2
    background = visual.NoiseStim(mywin, noiseType='binary', opacity=1, size=[screenSize[0] * backgroundScaleFactor, screenSize[1] * backgroundScaleFactor],
                                  noiseElementSize=dotSize_pixels, units=units, contrast=100 / 100)
    ...
    for ii in range(nFrames):
        randomCircleIndices.append(random.randint(0,nRandomIterations-1))
    ...
    circle.nElements = len(circleIndicesToIterate[0])
    circle.sizes = sizes[circleIndicesToIterate[0]]
    circle.oris = oris[circleIndicesToIterate[0]]
    circle.opacities = opacities[circleIndicesToIterate[0]]
    circle.sfs = sfs[circleIndicesToIterate[0]]
    circle.phases = phases[circleIndicesToIterate[0]]

    circle.verticesBase = verticesBase[circleIndicesToIterate[0]]
    circle.verticesPix = verticesPix[circleIndicesToIterate[0]]

    for i in range(nFrames):

        background.pos = randomBackgroundOrigins[i]
        background.draw()

        randomIndex = randomCircleIndices[i]

        circle.xys = circle_xys[circleIndicesToIterate[randomIndex]]
        circle.colors = colors[circleIndicesToIterate[randomIndex]]



        circle.fieldPos = [xPosition[i], yPosition[i]]  # this will make the thing vertically
        #circle.setXYs(circle.xys, '+')
        #circle.xys = circle.xys + 1
        circle.draw()
